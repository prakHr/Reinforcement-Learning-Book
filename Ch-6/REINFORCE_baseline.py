...
def REINFORCE_baseline(env_name, hidden_sizes=[32], p_lr=5e-3, vf_lr=8e-3, num_epochs=50, gamma=0.99, steps_per_epoch=100):
    '''
    REINFORCE Algorithm
    ...
    for ep in range(num_epochs):
        ...
        # initiailize buffer and other variables for the new epochs
        buffer = Buffer(gamma)
        env_buf = []
        ep_rews = []
        ...
        while len(buffer) < steps_per_epoch:
            ...
            # add the new transition, included the state value predictons
        ...
        # Optimize the policy
        sess.run(p_opt, feed_dict={obs_ph:obs_batch, act_ph:act_batch, ret_ph:ret_batch,rtg_ph:rtg_batch})
...
if __name__ == '__main__':
    REINFORCE_baseline('LunarLander-v2', hidden_sizes=[64],  p_lr=8e-3, vf_lr=7e-3, gamma=0.99, num_epochs=1000, steps_per_epoch=1000)
